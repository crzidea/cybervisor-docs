...
- **`thinking:`** — Internal model reasoning (Claude `ThinkingBlock`, OpenCode reasoning). Multiline thinking uses the same blank-line-and-indent format as replies, with original leading whitespace preserved per content line. Single-line thinking uses the inline format `thinking: text`. For Claude, only true thinking blocks are labeled `thinking:` — visible assistant text is always `reply:`, never `thinking:`.
...
- **Task** (subagent dispatch) prints the full `description` and `prompt` with `subject` as a rendered field. When an ACP adapter maps a subagent tool through the Task formatter, the first-line tool label may still show the agent’s name while the argument block includes the complete prompt.
