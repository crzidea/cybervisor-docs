...
While a stage runs, cybervisor prints the agent’s activity to your terminal and records per-stage logs under `.cybervisor/logs/`. `tool call:` lines show the same style of path, command, and search summaries for Claude and for ACP-based agents (Gemini, Codex, Cursor), serve-based agents (OpenCode), and in-process SDK agents (Antigravity) when the agent reports usable arguments. The tool title on the first line may still read like that agent’s UI label (for example Cursor’s “Read File” or “Find”) while the argument lines follow the same layout as other agents. If the agent sends a tool event without arguments, the line may show only the tool label. For OpenCode specifically, bare tool-call start events (without arguments) are deferred in favor of the next parameterized event for the same call, so you see one meaningful line per tool call rather than duplicate starts. OpenCode lifecycle and metadata events (“server.connected”, “session.next.agent.switched”, “session.next.model.switched”, “todo.updated”, “step-start”, “step-finish”) are suppressed from stderr; they are still recorded in the per-stage JSONL log for debugging.

- **TodoWrite** entries list every item with its full subject text — no truncation on item count or subject length.
- **Task** (subagent dispatch) prints the full `description` and `prompt` without the 80-character truncation used for most other tools. When an ACP adapter maps a subagent tool through the Task formatter, the first-line tool label may still show the agent's name while the argument block includes the complete prompt.
...
- Non-mock runs restore the pre-run `.claude/settings.json` content during cleanup (Gemini, Codex, and Cursor do not use settings-file patching — Gemini and Cursor use Strategy B with ACP runtime permission enforcement; OpenCode uses serve-mode HTTP with native permission config via `OPENCODE_CONFIG_CONTENT` and does not patch settings files)
- Hook runtime metadata under `.cybervisor/hooks/` is non-secret; verifier credentials remain in `~/.cybervisor/config.yaml`
- **Enforcement-mode marker**: For ACP agents (Gemini, Cursor), an `enforcement_mode` event is written to `.cybervisor/hooks/hook-events.jsonl` at session start. Value `"proactive"` means the agent is emitting `session/request_permission` events and ACP-level tool calls can be denied before execution. Value `"post_hoc_only"` means ACP permission requests are unavailable and Cybervisor relies on other layers for protection. OpenCode applies native permission rules via `OPENCODE_CONFIG_CONTENT` in serve mode; Cursor applies native deny rules via `.cursor/cli.json` and uses post-hoc filesystem snapshot restoration as a backstop for `read_only_paths`. The marker does not reflect those native harness permissions.
